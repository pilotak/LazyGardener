...
barometer = BMP085({address: config.meteo_station.BMP085_addr, mode: 3, units: 'metric'});

var BH1750    = require('bh1750'),
light   = new BH1750({address: config.meteo_station.BH1750_addr, device: config.i2c.dev, command: 0x10, length: 2});
...
new Cron(config.meteo_station.interval, function(){
  ...
  function(err, results){
    if (!err) {
      db.writePoint("meteo", results, function(err, response) {
        if(!err) {
